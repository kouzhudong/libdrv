...
Set-Location "$root\01-kernel-lib"
...
Set-Location "$root\02-rust-sys"
cargo build --release
if ($LASTEXITCODE) { throw "rust-sys 失败" }
Copy-Item "$root\02-rust-sys\target\release\rust_sys.dll" "$root\02-rust-sys\target\release\rust_sys.sys" -Force
...
& $msbuild "$root\03-cpp-wdm-sys\cpp-wdm-sys.vcxproj" /p:Configuration=Release /p:Platform=x64 /nologo /v:minimal
if ($LASTEXITCODE) { throw "cpp-wdm-sys 失败" }

Write-Host "`n全部完成。产物:" -ForegroundColor Green
Write-Host "  01: $root\01-kernel-lib\target\release\kernel_lib.lib  (+ libkernel_lib.rlib)"
Write-Host "  02: $root\02-rust-sys\target\release\rust_sys.sys"
Write-Host "  03: $root\03-cpp-wdm-sys\x64\Release\cpp-wdm-sys.sys"
