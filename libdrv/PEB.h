...
typedef struct _PEB32 {
    ...
    PEBTEB_POINTER(/*HANDLE*/ unsigned int)
    Mutant; // INITIAL_PEB structure is also updated.

    PEBTEB_POINTER(/*PVOID*/ unsigned int)
    ImageBaseAddress;
    PEBTEB_POINTER(/*PPEB_LDR_DATA PEB_LDR_DATA_WOW*/ unsigned int)
    Ldr;
    PEBTEB_POINTER(struct _RTL_USER_PROCESS_PARAMETERS *)
    ProcessParameters;
    PEBTEB_POINTER(PVOID)
    SubSystemData;
    PEBTEB_POINTER(PVOID)
    ProcessHeap;
    PEBTEB_POINTER(struct _RTL_CRITICAL_SECTION *)
    FastPebLock;
    PEBTEB_POINTER(PVOID)
    AtlThunkSListPtr; // Used only for AMD64
    PEBTEB_POINTER(PVOID)
    SparePtr2;
    ...
    ULONG SpareUlong;
    PEBTEB_POINTER(PPEB_FREE_BLOCK)
    FreeList;
    ...
    PEBTEB_STRUCT(GDI_HANDLE_BUFFER)
    GdiHandleBuffer;
    PEBTEB_POINTER(PPS_POST_PROCESS_INIT_ROUTINE)
    ...
    // Fusion stuff
    //
    PEBTEB_POINTER(const struct _ACTIVATION_CONTEXT_DATA *)
    ActivationContextData;
    PEBTEB_POINTER(struct _ASSEMBLY_STORAGE_MAP *)
