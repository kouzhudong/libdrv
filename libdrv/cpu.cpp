...
*/
{
    ...
    __cpuid(CPUInfo, 1);
    EDX = CPUInfo[3];
    if (_bittest(reinterpret_cast<long *>(&EDX), 6)) {
...
*/
{
    ...
    unsigned char b = _bittest(reinterpret_cast<const LONG *>(&edx), 5);
    if (b) {
...
*/
{
    ...
    t = __readmsr(0x482);
    if (_bittest64(reinterpret_cast<LONG64 const *>(&t), 63) == 0) {
...
*/
{
    ...
    if (_bittest64(reinterpret_cast<LONG64 const *>(&t), 33) || _bittest64(reinterpret_cast<LONG64 const *>(&t), 37)) {
        NOTHING;
    } else {
        return;
    }
    ...
    for (;; Index += 1) {
        ...
        */
        ...
        if (CacheSize >= (1024 * 1024)) {
            printf_s("   CacheSize = %lluMB.\n", CacheSize / (1024 * 1024));
        } else {
            printf_s("   CacheSize = %lluKB.\n", CacheSize / 1024);
        }
    }
