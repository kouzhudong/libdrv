...
__forceinline struct _TEB * NTAPI NtCurrentTeb(VOID)
{
    return reinterpret_cast<struct _TEB *>(__readgsqword(FIELD_OFFSET(NT_TIB, Self)));
}

__forceinline PVOID GetCurrentFiber(VOID)
{
    return reinterpret_cast<PVOID>(__readgsqword(FIELD_OFFSET(NT_TIB, FiberData)));
}

__forceinline PVOID GetFiberData(VOID)
{
    return *reinterpret_cast<PVOID *>(GetCurrentFiber());
}
...
__inline struct _TEB * NTAPI NtCurrentTeb(VOID)
{
