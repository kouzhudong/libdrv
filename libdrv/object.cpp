...
*/
{
    ...
    if (nullptr == Object || nullptr == ObjectName) {
        return STATUS_INVALID_PARAMETER;
    }

    RtlInitEmptyUnicodeString(ObjectName, nullptr, 0);
...
*/
{
    ...
    if (nullptr == SymbolicLinkName || nullptr == NtPathName || nullptr == DosPathName) {
        return STATUS_INVALID_PARAMETER;
    }

    do {
        InitializeObjectAttributes(&ObjectAttributes, SymbolicLinkName, OBJ_CASE_INSENSITIVE | OBJ_KERNEL_HANDLE, nullptr, nullptr);
        ...
        if (!NT_SUCCESS(Status) || nullptr == NtPathName->Buffer) {
            PrintEx(DPFLTR_DEFAULT_ID, DPFLTR_ERROR_LEVEL, "Error: Status:%#x", Status);
            break;
        }
...
*/
{
    ...
    NTSTATUS Status = ZwOpenDirectoryObject(&FileHandle, GENERIC_READ | SYNCHRONIZE, &ob);
    if (!NT_SUCCESS(Status)) {
        Print(DPFLTR_DEFAULT_ID, DPFLTR_ERROR_LEVEL, "0x%#x", Status);
        return Status;
    }
...
*/
{
    ...
    for (int x = 0;; x++) {
        Status = ZwEnumerateTransactionObject(nullptr, KTMOBJECT_TRANSACTION, &Cursor, sizeof(Cursor), &ReturnedBytes);
    ...
    for (int x = 0;; x++) {
        Status = ZwEnumerateTransactionObject(nullptr, KTMOBJECT_TRANSACTION_MANAGER, &Cursor, sizeof(Cursor), &ReturnedBytes);
        if (STATUS_NO_MORE_ENTRIES == Status) {
            break;
        }
...
注释：
1.这个是比较标准和安稳的办法。
2.要想设置句柄的GrantedAccess，可以遍历句柄表。
  富贵险中求，此话不假，这里尽是未公开的结构和函数。
3.
*/
{
    NTSTATUS Status = STATUS_UNSUCCESSFUL;
...
#pragma prefast(pop)
        {
            ...
            PPUBLIC_OBJECT_TYPE_INFORMATION ppoti{};
...
*/
{
    NTSTATUS Status = AdjustPrivilege(SE_DEBUG_PRIVILEGE, TRUE);
    if (!NT_SUCCESS(Status)) {
        Print(DPFLTR_DEFAULT_ID, DPFLTR_ERROR_LEVEL, "0x%#x", Status);
        return Status;
    }
    ...
    HANDLE SystemProcess = PsGetProcessId(PsInitialSystemProcess);
    Status = EnumerateProcessHandles(SystemProcess, &ProcessHandles);
    if (!NT_SUCCESS(Status)) {
        Print(DPFLTR_DEFAULT_ID, DPFLTR_ERROR_LEVEL, "0x%#x", Status);
        return Status;
    }
