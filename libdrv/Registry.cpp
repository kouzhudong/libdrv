...
*/
{
    ...
    */
    ...
    for (ULONG i = 0; i < pfi->SubKeys; i++) {
        ...
        new_key.Buffer = (wchar_t *)ExAllocatePoolWithTag(NonPagedPool, MAX_PATH * sizeof(WCHAR), TAG);
        if (new_key.Buffer == nullptr) {
            ExFreePoolWithTag(pbi, TAG);
            Status = STATUS_INSUFFICIENT_RESOURCES;
            break;
        }
        RtlZeroMemory(new_key.Buffer, MAX_PATH * sizeof(WCHAR));
        RtlInitEmptyUnicodeString(&new_key, new_key.Buffer, MAX_PATH * sizeof(WCHAR));
...
*/
{
    ...
    do {
        ...
        if (CompleteName->Buffer[0] == L'\\') {
            ...
            if (L'\\' != KeyFullName->Buffer[KeyFullName->Length / sizeof(WCHAR) - 1]) { // 判断结尾是否带斜杠。
