...
*/
{
    ...
    */
    ...
    if (!NT_SUCCESS(Status)) {
        ...
        if (NT_SUCCESS(Status) || BufferSizeNeeded == 0) {
            return Status;
        }
...
void PrintVolumeStandardInformation(PFLT_VOLUME Volume)
{
    ...
    VolumeName.MaximumLength = pvsi->FilterVolumeNameLength;
...
void EnumerateInstances(PFLT_FILTER Filter)
{
    ...
    InstanceListSize = sizeof(PFLT_INSTANCE) * NumberInstancesReturned * 2; 
...
void EnumerateVolumes(PFLT_FILTER Filter)
{
    ...
    VolumeListSize = sizeof(PFLT_VOLUME) * NumberVolumesReturned * 2; 
...
NTSTATUS EnumerateFilters()
{
    ...
    FilterListSize = sizeof(PFLT_FILTER) * NumberFiltersReturned * 2; 
