...
*/
{
    ...
    if ((FunctionAddress > (PVOID)ExportDirectory) && (FunctionAddress < (PVOID)((PCHAR)ExportDirectory + ExportSize))) {
        PrintEx(DPFLTR_DEFAULT_ID, DPFLTR_WARNING_LEVEL, "%s", "Forwarded export encountered — not supported");
        return nullptr;
    }
...
#pragma prefast(pop)
        ...
        if (pmdl->ByteCount < 8) {
            PrintEx(DPFLTR_DEFAULT_ID, DPFLTR_ERROR_LEVEL, "MDL too small: ByteCount=%u, need 8", pmdl->ByteCount);
            __leave;
        }
...
#pragma prefast(pop)
        ...
        if (pmdl->ByteCount < 6) {
            PrintEx(DPFLTR_DEFAULT_ID, DPFLTR_ERROR_LEVEL, "MDL too small: ByteCount=%u, need 6", pmdl->ByteCount);
            __leave;
        }
...
*/
{
    ...
    if (!NT_SUCCESS(Status)) {
        PrintEx(DPFLTR_DEFAULT_ID, DPFLTR_ERROR_LEVEL, "LdrFindResource_U failed: %#x", Status);
        ZwClose(DestinationFileHandle);
        return Status;
    }
    ...
    if (!NT_SUCCESS(Status)) {
        PrintEx(DPFLTR_DEFAULT_ID, DPFLTR_ERROR_LEVEL, "LdrAccessResource failed: %#x", Status);
        ZwClose(DestinationFileHandle);
        return Status;
    }
    ...
    if (!NT_SUCCESS(Status)) {
        PrintEx(DPFLTR_DEFAULT_ID, DPFLTR_ERROR_LEVEL, "ZwWriteFile failed: %#x", Status);
    }
