...
*/
{
    ...
    auto modules = (PAUX_MODULE_EXTENDED_INFO)ExAllocatePoolWithTag(PagedPool, modulesSize, TAG);
    if (modules == nullptr) {
...
*/
{
    ...
    for (ULONG i = 0; i < numberOfModules; i++) {
#pragma prefast(push)
#pragma prefast(disable : 6385, "XXX")
        UCHAR * FileName = modules[i].FullPathName + modules[i].FileNameOffset;
#pragma prefast(pop)
...
{
    PLOAD_IMAGE_CONTEXT ctx = reinterpret_cast<PLOAD_IMAGE_CONTEXT>(Parameter);
