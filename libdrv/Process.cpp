...
DWORD GetProcessIntegrityLevel(_In_ HANDLE UniqueProcess)
{
    ...
    if (!NT_SUCCESS(Status)) {
        PrintEx(DPFLTR_DEFAULT_ID, DPFLTR_ERROR_LEVEL, "ZwOpenProcess failed: %#x", Status);
        return 0;
    }
    ...
    if (!NT_SUCCESS(Status)) {
        PrintEx(DPFLTR_DEFAULT_ID, DPFLTR_ERROR_LEVEL, "ZwOpenProcessTokenEx failed: %#x", Status);
    ...
    if (Status != STATUS_BUFFER_TOO_SMALL || !ReturnLength) {
        PrintEx(DPFLTR_DEFAULT_ID, DPFLTR_ERROR_LEVEL, "ZwQueryInformationToken sizing failed: %#x", Status);
        ZwClose(TokenHandle);
        ZwClose(ProcessHandle);
        return 0;
    }
...
*/
{
    ...
    if (!NT_SUCCESS(Status)) {
        PrintEx(DPFLTR_DEFAULT_ID, DPFLTR_ERROR_LEVEL, "ObReferenceObjectByHandle(Process) failed: %#x", Status);
        ZwClose(TokenHandle);
        ZwClose(ProcessHandle);
        return 0;
    }
    ...
    if (!NT_SUCCESS(Status)) {
        PrintEx(DPFLTR_DEFAULT_ID, DPFLTR_ERROR_LEVEL, "ObReferenceObjectByHandle(Token) failed: %#x", Status);
