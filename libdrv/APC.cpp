...
*/
{
    ...
    st = ObReferenceObjectByHandle(ThreadHandle, THREAD_SET_CONTEXT, *PsThreadType, Mode, (PVOID *)&Thread, nullptr);
    if (NT_SUCCESS(st)) {
        st = STATUS_SUCCESS;
...
//注意：这个函数应在用户空间。
{
    auto pfnAPC = static_cast<PAPCFUNC>(lpApcArgument1);
    auto dwData = reinterpret_cast<ULONG_PTR>(lpApcArgument2);
